{% macro mo_aggr_info_upsert_s19_250() %}

-- Step 1: Merge (Update + Insert)
MERGE `{{ var('Redshift_gdap_Schema') }}.mo_aggr_info` T
USING `{{ var('Redshift_gdap_Utility') }}.mo_aggr_info_dypt_sls` S
ON T.YR_NU = S.YR_NU
   AND T.MO_NU = S.MO_NU
...
WHEN MATCHED THEN
  UPDATE SET
    T.MO_DYPT_SLS_VRNC_PC      = S.MO_DYPT_SLS_VRNC_PC,
    T.MO_AGGR_DYPT_SLS_DY_QT   = S.MO_AGGR_DYPT_SLS_DY_QT,
    T.MO_AGGR_DYPT_7DY_RULE_FL = S.MO_AGGR_DYPT_7DY_RULE_FL,
...
WHEN NOT MATCHED THEN
  ...
  VALUES (
    ...
    S.MO_DYPT_SLS_VRNC_PC,
    S.MO_AGGR_DYPT_SLS_DY_QT,
    S.MO_AGGR_DYPT_7DY_RULE_FL,
    0
  );
