{% macro mo_aggr_info_tm_seg_sls_upsert_s19_230() %}

-- Step 1: Merge (Update + Insert)
MERGE `{{ var('Redshift_gdap_Schema') }}.mo_aggr_info` T
USING `{{ var('Redshift_gdap_Utility') }}.mo_aggr_info_tm_seg_sls` S
ON T.YR_NU = S.YR_NU
   AND T.MO_NU = S.MO_NU
...
WHEN MATCHED THEN
  UPDATE SET
    T.MO_TM_SEG_SLS_VRNC_PC    = S.MO_TM_SEG_SLS_VRNC_PC,
    T.MO_AGGR_TM_SEG_SLS_DY_QT = S.MO_AGGR_TM_SEG_SLS_DY_QT,
...
WHEN NOT MATCHED THEN
  ...
  VALUES (
    ...
    S.MO_TM_SEG_SLS_VRNC_PC,
    S.MO_AGGR_TM_SEG_SLS_DY_QT
  );
...
AND T.MO_TM_SEG_SLS_VRNC_PC    IS NULL
AND T.MO_AGGR_TM_SEG_SLS_DY_QT IS NULL;
