{% macro delete_mo_tm_seg_sls() %}
    ...
    {# Step 2: Build and run delete statement #}
    {% set delete_sql %}
        DELETE FROM `{{ var('Redshift_gdap_Schema') }}.mo_tm_seg_sls` AS target
        WHERE EXISTS (
            SELECT 1
            FROM `{{ var('Redshift_gdap_Schema') }}.cal_dt` AS cal
            WHERE cal.cal_dt BETWEEN DATE('{{ MO_FROM_DT }}') AND DATE('{{ MO_TO_DT }}')
              AND cal.YR_NU = target.YR_NU
              AND cal.MO_NU = target.MO_NU
        )
    {% endset %}
