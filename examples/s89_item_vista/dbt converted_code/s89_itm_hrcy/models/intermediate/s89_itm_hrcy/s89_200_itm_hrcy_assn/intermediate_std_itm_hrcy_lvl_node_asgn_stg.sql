...
{% if execute %}
  {% do run_query(REUSABLE_JS_S3_REDSHIFT_copy_withdynamicschema_new_API_updated(
      ...
      target_table_name='std_itm_hrcy_lvl_node_asgn',
      source_dataset_name='MCD_DBT_IV',
      source_table_name='intermediate_std_itm_hrcy_lvl_node_asgn',
      is_delete=1
  )) %}
{% endif %}

SELECT * FROM {{ ref('intermediate_std_itm_hrcy_lvl_node_asgn') }}