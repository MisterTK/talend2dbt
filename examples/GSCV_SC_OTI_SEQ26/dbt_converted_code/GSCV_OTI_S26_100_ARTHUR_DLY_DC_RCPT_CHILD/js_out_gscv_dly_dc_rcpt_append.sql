...
SELECT
  ...
  t1.fdc_case_cost_am,
  t1.raw_itm_lcl_fl,
  t1.gbal_fca_case_cost_am,
  t1.gbal_fdc_case_cost_am,
  t1.gbal_curn_iso_nu,
  t1.dw_file_id,
  t1.dc_gln_id,
  t1.dc_trsf_fcil_gln_id,
  ...
  t1.fcil_pri_gln_id,
  t1.gbal_trad_itm_nu,
  -- Euro currency conversion calculation for fca
  CASE
    WHEN t1.gbal_fca_case_cost_am IS NULL OR t2.dly_xcng_rate_nu IS NULL THEN NULL
    ELSE t1.gbal_fca_case_cost_am / t2.dly_xcng_rate_nu
  END AS euro_fca_case_cost_am,
  -- Euro currency conversion calculation for fdc
  CASE
    WHEN t1.gbal_fdc_case_cost_am IS NULL OR t2.dly_xcng_rate_nu IS NULL THEN NULL
    ELSE t1.gbal_fdc_case_cost_am / t2.dly_xcng_rate_nu
...
SELECT
  ...
  t1.dc_trsf_fcil_id,
  t1.shpg_unt_case_recv_qt,
  CAST(t1.fca_case_cost_am AS NUMERIC) AS fca_case_cost_am,
  CAST(t1.fdc_case_cost_am AS NUMERIC) AS fdc_case_cost_am,
  t1.raw_itm_lcl_fl,
  CAST(t1.gbal_fca_case_cost_am AS NUMERIC) AS gbal_fca_case_cost_am,
  CAST(t1.gbal_fdc_case_cost_am AS NUMERIC) AS gbal_fdc_case_cost_am,
  t1.gbal_curn_iso_nu,
  t1.dc_gln_id,
  t1.dc_trsf_fcil_gln_id,
  t1.fcil_gln_id,
  ...
  t1.fcil_pri_gln_id,
  t1.gbal_trad_itm_nu,
  CAST(t1.euro_fca_case_cost_am AS NUMERIC) AS euro_fca_case_cost_am,
  CAST(t1.euro_fdc_case_cost_am AS NUMERIC) AS euro_fdc_case_cost_am,
  t1.euro_curn_iso_nu,
  TRIM(t1.dw_file_id) AS dw_file_id,
  t1.Timestamp
FROM
  Dly_Dc_Rcpt_Valid_3 AS t1
INNER JOIN
  {{ source('rmdw_tables','raw_itm') }} AS t2
ON
  t1.terr_cd_mapped = t2.terr_cd
  AND t1.wrin = t2.xtrn_raw_itm_nu